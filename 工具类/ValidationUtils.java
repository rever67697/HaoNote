...
//^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).{8,10}$
	...
	public static boolean isIDCard(String IDCardNo) {
		...
		if (IDCardNo.length() != 15 && IDCardNo.length() != 18) {
			...
			AppLogMessageMgr.e("ValidationUtils-->>isIDCard", errmsg);
			return false;
		}
		...
		try {
			...
			e.printStackTrace();
			errmsg = "身份证生日不在有效范围";
			AppLogMessageMgr.e("ValidationUtils-->>isIDCard", errmsg  + e.getMessage());
			return false;
		} catch (java.text.ParseException e1) {
			e1.printStackTrace();
			errmsg = "身份证生日不在有效范围";
			AppLogMessageMgr.e("ValidationUtils-->>isIDCard", errmsg + e1.getMessage());
			return false;
		}
